...
/**
 * Parse text with **bold** markers and render with appropriate styling
 */
      ...
      if (numberedMatch) {
        return (
          <div key={index} className="flex gap-2 ml-4">
            ...
            <span>{renderTextWithBold(numberedMatch[2])}</span>
          </div>
        );
      }
      ...
      if (trimmedLine.startsWith('•') || trimmedLine.startsWith('-')) {
        ...
        return (
          <div key={index} className="flex gap-2 ml-4">
            ...
            <span>{renderTextWithBold(bulletText)}</span>
          </div>
        );
      }
      ...
      if (line.startsWith('  ') && (trimmedLine.startsWith('-') || trimmedLine.startsWith('•'))) {
        ...
        return (
          <div key={index} className="flex gap-2 ml-8">
            <span className="flex-shrink-0">-</span>
            <span className="text-sm text-primary-600">{renderTextWithBold(bulletText)}</span>
          </div>
        );
      }

      // Regular text - check for bold markers
