...
import { Button, TextArea, Toggle, Input } from '../../common';
...
export function ConflictsStep() {
  const { data, updateConflicts, addConflict, removeConflict, updateConflictMitigations } = useWizardStore();
  const conflicts = data.conflicts || [];
  const conflictMitigations = data.conflictMitigations;
  ...
  const handleAddMitigation = () => {
    ...
    const currentStrategies = conflictMitigations?.mitigationStrategies || [];
    updateConflictMitigations({
      mitigationStrategies: [...currentStrategies, newMitigationText.trim()],
    });
  ...
  const handleRemoveMitigation = (index: number) => {
    const currentStrategies = conflictMitigations?.mitigationStrategies || [];
    updateConflictMitigations({
      mitigationStrategies: currentStrategies.filter((_, i) => i !== index),
    });
  };

  const handleUpdateMitigation = (index: number, value: string) => {
    const currentStrategies = conflictMitigations?.mitigationStrategies || [];
    const updated = [...currentStrategies];
    updated[index] = value;
    updateConflictMitigations({ mitigationStrategies: updated });
  };
  ...
  return (
    <WizardStepContent>
      <div className="space-y-8">
        ...
        {/* Conflict Mitigations Section */}
        <WizardStepSection
          title="Conflict Mitigation Strategies"
          description="Explain how you mitigate conflicts of interest. These strategies will be included after your conflict disclosures."
        >
          <div className="space-y-4">
            <div className="bg-secondary-50 border border-secondary-200 rounded-lg p-4">
              <Toggle
                ...
                checked={conflictMitigations?.includeMitigations || false}
                onChange={(e) =>
                  updateConflictMitigations({ includeMitigations: e.target.checked })
                }
              />
            </div>

            {conflictMitigations?.includeMitigations && (
              <div className="space-y-4">
                {/* Current Mitigation Strategies */}
                <div className="space-y-2">
                  {(conflictMitigations?.mitigationStrategies || []).map((strategy, index) => (
                    <div key={index} className="flex gap-2 items-start">
                      <div className="flex-1">
                        <Input
                          value={strategy}
                          onChange={(e) => handleUpdateMitigation(index, e.target.value)}
                ...
                <div className="flex gap-2">
                  <div className="flex-1">
                    <Input
                      ...
                      placeholder="Add a new mitigation strategy..."
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          e.preventDefault();
                          handleAddMitigation();
                  ...
                  <Button onClick={handleAddMitigation} disabled={!newMitigationText.trim()}>
                    Add
                  </Button>
                </div>
                ...
                {/* Preview */}
                {(conflictMitigations?.mitigationStrategies || []).length > 0 && (
                  <div className="bg-primary-50 rounded-lg p-4 border border-primary-200">
                    <h4 className="text-sm font-medium text-primary-700 mb-2">
                      Letter Preview
                    </h4>
                    <p className="text-sm text-primary-600 italic mb-2">To mitigate these conflicts:</p>
                    <ul className="text-sm text-primary-600 italic space-y-1">
                      {(conflictMitigations?.mitigationStrategies || []).map((strategy, index) => (
